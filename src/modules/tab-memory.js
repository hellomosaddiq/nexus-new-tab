 ...
 *
 * Advanced AI-powered user behavior tracking and prediction system with
 * sophisticated machine learning algorithms, statistical analysis, and
 * privacy-first local analytics for optimal user experience personalization.
 *
 * CORE AI FEATURES:
 * - Machine learning-based tab prediction with confidence scoring
 * - Adaptive threshold algorithms that learn user consistency patterns
 * - Time-weighted scoring with exponential decay functions
 * - Contextual intelligence with time-of-day pattern recognition
 * - Burst activity detection for immediate behavior adaptation
 * - Statistical variance analysis for user consistency measurement
 *
 * ADVANCED ALGORITHMS:
 * - Exponential decay scoring for temporal relevance weighting
 * - Multi-factor confidence calculation with research-backed weights
 * - Adaptive threshold adjustment based on usage variance analysis
 * - Logarithmic burst bonus scaling with diminishing returns
 * - Consistency bonus calculation for regular usage patterns
 * - Cross-temporal pattern recognition for contextual predictions
 *
 * PRIVACY & PERFORMANCE:
 * - 100% local processing - no data leaves the user's device
 * - Efficient storage compression with 1MB soft limit
...
class TabMemorySystem {
    ...
    async loadMemoryData() {
        try {
            ...
            if (api?.storage?.local) {
                ...
                this.memoryData = this.validateMemoryData(result[this.storageKey]) || this.getDefaultMemoryData();
                this.analytics = this.validateAnalytics(result[this.analyticsKey]) || this.getDefaultAnalytics();
                ...
                this.memoryData = this.validateMemoryData(parsedMemoryData) || this.getDefaultMemoryData();
                this.analytics = this.validateAnalytics(parsedAnalytics) || this.getDefaultAnalytics();
            }
    ...
    validateMemoryData(data) {
        ...
        try {
            ...
            return {
                ...
                tabUsage: data.tabUsage && typeof data.tabUsage === 'object' ? data.tabUsage : defaultData.tabUsage,

                // Preserve valid preferences object or use default
                preferences: data.preferences && typeof data.preferences === 'object' ? data.preferences : defaultData.preferences
            };
        } catch (error) {
    ...
    validateAnalytics(data) {
        ...
        try {
            ...
            return {
                ...
                totalSessions: typeof data.totalSessions === 'number' ? data.totalSessions : defaultAnalytics.totalSessions,
                averageSessionLength: typeof data.averageSessionLength === 'number' ? data.averageSessionLength : defaultAnalytics.averageSessionLength,

                // Handle optional fields with undefined checking
                mostUsedTimeOfDay: data.mostUsedTimeOfDay !== undefined ? data.mostUsedTimeOfDay : defaultAnalytics.mostUsedTimeOfDay,

                // Validate object structures with fallbacks
                patterns: data.patterns && typeof data.patterns === 'object' ? data.patterns : defaultAnalytics.patterns
            };
        } catch (error) {
            // ===== COMPLETE FAILURE FALLBACK =====
            return null;
    ...
    async recordInteraction(tabType, action = 'click', context = {}) {
        try {
            ...
            if (this.memoryData.sessions.length > this.config.maxHistoryEntries) {
                this.memoryData.sessions = this.memoryData.sessions.slice(-this.config.maxHistoryEntries);
            }
    ...
    calculateMostUsedTimeOfDay() {
        ...
        for (const hour in this.analytics.patterns) {
            ...
            const hourlyUsage = Object.values(this.analytics.patterns[hour])
    ...
    async calculatePreferences() {
        ...
        for (const [tabType, usage] of Object.entries(this.memoryData.tabUsage)) {
            ...
            const recentSessions = this.memoryData.sessions.filter(session =>
            ...
            const recentDays = 7;
            const daysSinceFirstUse = Math.min(recentDays,
                (Date.now() - (this.memoryData.sessions.find(s => s.tabType === tabType)?.timestamp || Date.now()))
                / (24 * 60 * 60 * 1000)
            );
    ...
    calculateUsageVariance() {
        ...
        const variance = distribution.reduce((sum, prob) => sum + Math.pow(prob - mean, 2), 0) / distribution.length;
    ...
    getPredictedTab() {
        ...
        const effectiveThreshold = this.memoryData.preferences.effectiveThreshold || this.config.confidenceThreshold;
    ...
    getAnalytics() {
        ...
        try {
            ...
            const dataSize = JSON.stringify(this.memoryData).length + JSON.stringify(this.analytics).length;
            ...
            return {
                ...
                preferences: this.memoryData.preferences || { preferredTab: 'bookmarks', confidence: 0 },
    ...
    async emergencyCleanup() {
        try {
            ...
            const currentHour = new Date().getHours();
            const relevantHours = [currentHour - 1, currentHour, currentHour + 1].map(h =>
                h < 0 ? h + 24 : h > 23 ? h - 24 : h  // Handle hour wraparound
            );
