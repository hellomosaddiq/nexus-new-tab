 ...
 *
 * Advanced command palette system with AI-powered tab prediction, intelligent
 * caching, and cross-browser compatibility. Inspired by modern productivity
 * tools like Raycast, Linear, Arc Browser, and Claude AI.
 *
 * CORE FEATURES:
 * - AI-powered tab prediction using machine learning algorithms
 * - Intelligent favicon caching with IndexedDB persistence
 * - Cross-browser API compatibility (Chrome, Firefox, Edge)
 * - Real-time search with fuzzy matching
 * - Adaptive UI with glassmorphism design
 * - Performance-optimized data loading and rendering
 *
 * ARCHITECTURE:
 * - Modular design with separate systems for memory, caching, and UI
 * - Event-driven architecture with proper cleanup
 * - Defensive programming with comprehensive error handling
 * - Memory-efficient with session-based caching
 *
 * DESIGN SYSTEM:
 * - 8px grid system for consistent spacing
 * - Glassmorphism with backdrop blur effects
 * - Dark theme with accent color integration
 * - Responsive design with touch-friendly interactions
 *
 * PERFORMANCE CONSIDERATIONS:
 * - Lazy loading of data sources
 * - Efficient DOM manipulation with virtual scrolling concepts
...
class QuickShortcuts {
    ...
    bindKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            // Block Ctrl+K to prevent browser address bar focus
            // This ensures our K shortcut works consistently
    ...
    async loadTopSites() {
        try {
            ...
            const realTopSites = await Promise.all(sites.map(async site => ({
        ...
        try {
            ...
            if (api?.sessions?.getRecentlyClosed) {
                try {
                    ...
                    recentTabs = await Promise.all(filteredSessions.map(async session => ({
            ...
            if (recentTabs.length < 6 && api?.history?.search) {
                try {
                    ...
                    const historyTabs = await Promise.all(sortedHistory.map(async item => ({
    ...
    async getFallbackSites(type) {
        ...
        const sites = await Promise.all(siteUrls.map(async site => ({
            title: site.title,
            url: site.url,
            favicon: await this.getCachedFavicon(site.url),    // Cached favicon loading
            type
    ...
    /**
     * Load developer-focused search engines
     *
     * Loads a curated collection of search engines optimized for developer
     * productivity and technical research. Each engine includes proper query
     * parameter handling and cached favicons.
     *
     * SEARCH ENGINE FEATURES:
     * - Developer-focused platforms (GitHub, Stack Overflow, MDN)
     * - Proper query parameter formatting
     * - Cached favicons for instant display
     * - Error handling with graceful degradation
     *
     * DEVELOPER PRODUCTIVITY:
     * - GitHub: Code and repository search
     * - Stack Overflow: Technical Q&A search
    ...
    async loadSearchEngines() {
        try {
            ...
            // Developer-focused search engines with proper query formatting

            const engines = [
                { name: 'GitHub', url: 'https://github.com/search?q=', domain: 'https://github.com' },
                { name: 'Stack Overflow', url: 'https://stackoverflow.com/search?q=', domain: 'https://stackoverflow.com' },
                { name: 'MDN', url: 'https://developer.mozilla.org/en-US/search?q=', domain: 'https://developer.mozilla.org' },
                { name: 'YouTube', url: 'https://youtube.com/results?search_query=', domain: 'https://youtube.com' },
                { name: 'Reddit', url: 'https://reddit.com/search?q=', domain: 'https://reddit.com' }
            ];

            // ===== CONCURRENT PROCESSING =====
            // Process all engines concurrently for optimal performance

            this.searchEngines = await Promise.all(engines.map(async engine => ({
                name: engine.name,
                url: engine.url,
                favicon: await this.getCachedFavicon(engine.domain)    // Cached favicon loading
            })));
        } catch (error) {
    ...
    getFaviconUrl(url) {
        try {
            ...
            const specialIcons = {
                ...
                'calendar.google.com': 'https://www.gstatic.com/images/branding/product/1x/calendar_32dp.png',

                // ===== CONTENT & MEDIA =====

                // YouTube (All Variants)
                'youtube.com': 'https://www.youtube.com/s/desktop/12d6b690/img/favicon_32x32.png',
                'www.youtube.com': 'https://www.youtube.com/s/desktop/12d6b690/img/favicon_32x32.png',
                ...
                'workspace.google.com': 'https://www.gstatic.com/images/branding/product/1x/workspace_32dp.png',
                'admin.google.com': 'https://www.gstatic.com/images/branding/product/1x/admin_32dp.png',

                // ===== CLOUD & DEVELOPER SERVICES =====

                // Google Cloud Platform
                'cloud.google.com': 'https://www.gstatic.com/images/branding/product/1x/cloud_32dp.png',
                'console.cloud.google.com': 'https://www.gstatic.com/images/branding/product/1x/cloud_32dp.png',
                'developers.google.com': 'https://www.gstatic.com/images/branding/product/1x/developers_32dp.png',

                // ===== MOBILE & APP ECOSYSTEM =====

                // Google Play & Android
                'play.google.com': 'https://www.gstatic.com/images/branding/product/1x/play_prism_64dp.png',
                'developer.android.com': 'https://www.gstatic.com/images/branding/product/1x/android_32dp.png',

                // ===== MARKETING & ANALYTICS =====

                // Google Analytics & Advertising
                'analytics.google.com': 'https://www.gstatic.com/images/branding/product/1x/analytics_32dp.png',
                'ads.google.com': 'https://www.gstatic.com/images/branding/product/1x/ads_32dp.png',
                'adsense.google.com': 'https://www.gstatic.com/images/branding/product/1x/adsense_32dp.png',
                ...
                // Google News & Books
                'news.google.com': 'https://www.gstatic.com/images/branding/product/1x/news_32dp.png',
                'books.google.com': 'https://www.gstatic.com/images/branding/product/1x/books_32dp.png',

                // ===== EXPLORATION & GEOGRAPHY =====

                // Google Earth & Geographic Services
                'earth.google.com': 'https://www.gstatic.com/images/branding/product/1x/earth_32dp.png',
                'mymaps.google.com': 'https://maps.gstatic.com/favicon3.ico',

                // ===== SOCIAL & COLLABORATION =====

                // Google Contacts & Groups
                'contacts.google.com': 'https://www.gstatic.com/images/branding/product/1x/contacts_32dp.png',
                'groups.google.com': 'https://www.gstatic.com/images/branding/product/1x/groups_32dp.png',
                ...
                'fonts.google.com': 'https://www.gstatic.com/images/branding/product/1x/fonts_32dp.png',

                // ===== LOCAL & BUSINESS =====

                // Google My Business & Local Services
                'business.google.com': 'https://www.gstatic.com/images/branding/product/1x/my_business_32dp.png',
                ...
                'chrome.google.com': 'https://www.gstatic.com/images/branding/product/1x/chrome_32dp.png',

                // ===== AI & FUTURE SERVICES =====

                // Google Gemini & AI Services
                'gemini.google.com': 'https://www.gstatic.com/lamda/images/gemini_favicon_f069958c85030456e93de685481c559f160ea06b.png',
                'bard.google.com': 'https://www.gstatic.com/lamda/images/gemini_favicon_f069958c85030456e93de685481c559f160ea06b.png'
            };
            ...
            // Domains known to cause issues with favicon loading
    ...
    getTabsHTML() {
        ...
        const tabs = [
            { id: 'bookmarks', label: 'Bookmarks', icon: this.getBookmarkIcon(), count: this.bookmarks.length },
            { id: 'topsites', label: 'Top Sites', icon: this.getStarIcon(), count: this.topSites.length },
            { id: 'recent', label: 'Recent', icon: this.getClockIcon(), count: this.recentTabs.length }
        ];
                ...
                ${items.map((item, index) => `
                    <div class="qs-item" data-url="${item.url}" data-session-id="${item.sessionId || ''}" tabindex="0">
                        ...
                        <div class="qs-item-content">
                            ...
                            <div class="qs-item-url">${this.getDomainName(item.url)}</div>
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    getCurrentItems() {
        switch (this.activeTab) {
            ...
            case 'topsites': return this.topSites;
            case 'recent': return this.recentTabs;
            default: return this.bookmarks;  // Safe fallback
        }
    }
    ...
    bindEvents(panel) {
        ...
        // Set up fallback icon handling for all images
    ...
    async testFaviconCaching() {
        ...
        if (this.cacheManager) {
            try {
                ...
                testResults.persistentCache.expired = cached ? this.cacheManager.isExpired(cached.timestamp, this.cacheManager.config.faviconExpiry) : null;
    ...
    async getCacheStats() {
        ...
        if (this.cacheManager) {
            try {
                ...
                const transaction = this.cacheManager.db.transaction([this.cacheManager.stores.favicons], 'readonly');
    ...
    async clearFaviconCache() {
        ...
        if (this.cacheManager && this.cacheManager.db) {
            try {
                const transaction = this.cacheManager.db.transaction([this.cacheManager.stores.favicons], 'readwrite');
                const store = transaction.objectStore(this.cacheManager.stores.favicons);
    ...
    async verifyCacheUsage() {
        ...
        for (const domain of testDomains) {
            ...
            results.tests.push({
                ...
                speedImprovement: firstCallTime > 0 ? ((firstCallTime - secondCallTime) / firstCallTime * 100).toFixed(1) + '%' : '0%'
            });
        }
