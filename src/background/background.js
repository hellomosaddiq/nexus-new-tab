    ...
    if (details.reason === 'install') {
        try {
            ...
            await api.storage.sync.set({
                ...
                smartDateFeatures: true,                     // Enable smart date insights
                selectedSmartDateFeatures: [                 // Start simple - let users discover more
                    'weekend-status'                         // Just weekend status - most universally useful
                ],

                // ===== PRODUCTIVITY FEATURES =====
                focusTimer: false,                           // Disabled by default
                quickNotes: false                            // Disabled by default
            });
        } catch (error) {
...
if (api.action && api.action.onClicked) {
    api.action.onClicked.addListener((tab) => {
        ...
        const isNewTab = tab.url.includes('chrome://newtab/') ||      // Chrome new tab
